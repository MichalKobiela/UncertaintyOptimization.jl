...
@model function fit(data::AbstractVector, prob, saveat::AbstractVector, distributions, ig;
    ...
    try
        ...
        warm = solve(prob, Rosenbrock23(), p=p_work, dtmin=1e-12, sensealg = SENSEALG)
        ...
        sol2 = solve(fast_prob, Rosenbrock23(), p=p_work; dtmin=1e-12, saveat=saveat, sensealg = SENSEALG)

        cuma_setter!(p_work, (0.001, ))
        sol3 = solve(fast_prob, Rosenbrock23(), p=p_work; dtmin=1e-12, saveat=saveat, sensealg = SENSEALG)
...
function perffit(prob, saveat::AbstractVector;
    force_values=nothing)
...
# f = open(string(@__DIR__)*"/minmtk_c32_jac1simplify.jls", "w")
